...
class VirtualPainter:
    def __init__(self):
        ...
        self.hands = mp.solutions.hands.Hands(max_num_hands=1, min_detection_confidence=0.75,
        ...
        self.tools = ["brush", "eraser", "rectangle", "circle", "line", "filled_rectangle", "filled_circle",
                      "clear_canvas"]
    ...
    def mouse_events(self, event, x, y, flags, param):
        if y < self.button_height + 20:
            ...
            if self.current_tool in ["rectangle", "circle", "line", "filled_rectangle", "filled_circle"]:
                self.draw_shape(self.canvas, self.current_tool, self.start_x, self.start_y, x, y, self.current_color,
    ...
    def draw_ui(self, frame):
        ...
        for i, color in enumerate(self.colors):
            ...
            cv2.rectangle(frame, (x_start, y_start), (x_start + self.color_button_width, y_start + self.button_height),
                          color, -1)
            if color == self.current_color:
                cv2.rectangle(frame, (x_start - 2, y_start - 2),
                              (x_start + self.color_button_width + 2, y_start + self.button_height + 2),
                              (255, 255, 255), 2)
        ...
        for i, tool in enumerate(self.tools):
            ...
            if tool == self.current_tool:
                cv2.rectangle(frame, (x_start, 10), (x_start + self.tool_button_width, self.button_height), (0, 255, 0),
                              2)
        ...
        cv2.putText(frame, f"{self.brush_thickness}", (brush_ui_x + 40, 65), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 0, 0),
                    2)
    ...
    def save_canvas(self):
        ...
        filename = f"{self.save_dir}/painting_{timestamp}.png"
        cv2.imwrite(filename, self.canvas)
        print(f"Saved painting as {filename}")

    def run(self):
        print("[INFO] Starting the virtual painter...")
        while True:
            ...
            if results.multi_hand_landmarks:
                ...
                for handLms in results.multi_hand_landmarks:
                    ...
                    if y1 < 150:
                        print("[DEBUG] UI interaction at y <", 150)
                        self.handle_ui_interaction(x1, y1)
                        self.drawing = False
                    elif pinch:
                        print("[DEBUG] Pinch detected at", (x1, y1))
                        if not self.drawing:
                            self.drawing = True
                            ...
                            self.temp_canvas = self.canvas.copy()
                            self.draw_shape(self.temp_canvas, self.current_tool, self.start_x, self.start_y, x1, y1,
                                            self.current_color, self.brush_thickness)
                    else:
                        if self.drawing and self.current_tool in ["rectangle", "circle", "line", "filled_rectangle",
                                                                  "filled_circle"]:
                            self.draw_shape(self.canvas, self.current_tool, self.start_x, self.start_y, x1, y1,
                                            self.current_color, self.brush_thickness)
                        self.drawing = False
            ...
            display = self.temp_canvas if self.drawing and self.current_tool in ["rectangle", "circle", "line",
                                                                                 "filled_rectangle",
                                                                                 "filled_circle"] else self.canvas
